...
function getAvailableQRCodes($pdo, $limit = 100) {
    ...
    $cacheKey = "available_qr_codes:{$limit}";
    ...
    if ($codes === null) {
        ...
        // 5 Minuten cachen (ändert sich häufiger)
        $cache->set($cacheKey, $codes, 300);
    }
...
// Seriennummer validieren (Zahlen, Buchstaben und Sonderzeichen)
function validateSerialNumber($serialNumber) {
    // Leer ist erlaubt
    if (empty($serialNumber)) {
        return true;
    }
    // Erlaubte Zeichen: Buchstaben (A-Z, a-z), Zahlen (0-9), 
    // und Sonderzeichen: - _ . / ( ) : , + * # und Leerzeichen
    // Länge: mindestens 1, maximal 100 Zeichen
    if (strlen($serialNumber) > 100) {
        return false;
    }
    return preg_match('/^[A-Za-z0-9\-_\.\/\(\)\:,\+\*\# ]+$/', $serialNumber);
}
...
function hasPermission($permissionKey) {
    ...
    // Nutze gecachte Permissions
    $permissions = getUserPermissions($_SESSION['user_id']);
    
    // Prüfe ob Permission in der Liste ist
...
// HTML escapen
function e($string) {
    // NULL-Werte abfangen (wichtig für PHP 8.1+)
...
function uploadImage($file, $markerId) {
    ...
    if (move_uploaded_file($file['tmp_name'], $filepath)) {
        ...
        // In uploaded_files Tabelle tracken
        if ($pdo) {
            try {
                $stmt = $pdo->prepare("INSERT INTO uploaded_files (marker_id, filename, original_filename, filepath, file_type, file_size, upload_category, uploaded_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([
                    $markerId,
                    $filename,
                    $file['name'],
                ...
                error_log('Fehler beim Tracking der hochgeladenen Datei: ' . $e->getMessage());
            }
        }
...
function getSystemSettings() {
    ...
    try {
        ...
        // 24 Stunden cachen (ändert sich sehr selten)
...
function getMarkerById($id, $pdo) {
    ...
    if ($marker === null) {
        ...
        if ($marker) {
            // 1 Stunde cachen (häufig aufgerufen)
            $cache->set($cacheKey, $marker, 3600);
        }
    }
...
function getAllMarkers($pdo) {
    ...
    // NUR cachen wenn KEINE Filter aktiv sind
    // Prüfe ob diese Funktion überhaupt noch verwendet wird!
    
    $cacheKey = "all_markers";
    $markers = $cache->get($cacheKey);
    ...
    if ($markers === null) {
        ...
        // 5 Minuten cachen
        $cache->set($cacheKey, $markers, 300);
    }
...
/**
 * Verfügbare NFC-Chips abrufen
 */
function getAvailableNFCChips($pdo, $limit = 100) {
    global $cache;
    $cacheKey = "available_nfc_chips_limit_$limit";
    ...
    $stmt = $pdo->prepare("
        SELECT nfc_chip_id, batch_name, created_at
        FROM nfc_chip_pool 
        WHERE is_assigned = 0 
        ORDER BY batch_name, created_at DESC 
        LIMIT ?
    ");
    $stmt->execute([$limit]);
    $result = $stmt->fetchAll();
    
    $cache->set($cacheKey, $result, 300); // 5 Minuten Cache
    return $result;
}

/**
 * NFC-Chip-ID validieren
 */
function validateNFCChipId($nfcChipId) {
    // NFC-Chip-IDs können verschiedene Formate haben (Hex, UID, etc.)
    // Erlaubt: Buchstaben, Zahlen, Bindestriche, Doppelpunkte
    if (empty($nfcChipId)) {
        return false;
    }
    
    // Maximale Länge: 100 Zeichen
    if (strlen($nfcChipId) > 100) {
        return false;
    }
    
    // Erlaubte Zeichen: A-Z, a-z, 0-9, Bindestrich, Doppelpunkt
    if (!preg_match('/^[A-Za-z0-9:\-]+$/', $nfcChipId)) {
        return false;
    }
...
/**
 * Prüfen ob NFC global aktiviert ist
 */
function isNFCEnabled() {
    global $pdo;
    $settings = getSystemSettings();
    return isset($settings['nfc_enabled']) && $settings['nfc_enabled'] == '1';
}

/**
 * Marker per NFC-Chip-ID laden
 */
function getMarkerByNFCChip($pdo, $nfcChipId) {
    $stmt = $pdo->prepare("
        SELECT m.*, u.username as created_by_name
        FROM markers m
        LEFT JOIN users u ON m.created_by = u.id
        WHERE m.nfc_chip_id = ? AND m.deleted_at IS NULL
    ");
...
/**
 * Verfügbare NFC-Chips zählen
 */
function countAvailableNFCChips($pdo) {
    $stmt = $pdo->query("SELECT COUNT(*) FROM nfc_chip_pool WHERE is_assigned = 0");
    return $stmt->fetchColumn();
}

/**
 * NFC-Chips in Pool generieren
 */
function generateNFCChipsToPool($pdo, $chipIds, $batchName = null) {
    try {
        $pdo->beginTransaction();
        
        $stmt = $pdo->prepare("
            INSERT INTO nfc_chip_pool (nfc_chip_id, batch_name, created_at)
            VALUES (?, ?, NOW())
        ");
        ...
        foreach ($chipIds as $chipId) {
            ...
            if (!validateNFCChipId($chipId)) {
                $errors[] = "Ungültige Chip-ID: $chipId";
                $skipCount++;
                continue;
            }
            
            // Prüfen ob bereits vorhanden
            $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM nfc_chip_pool WHERE nfc_chip_id = ?");
            $checkStmt->execute([$chipId]);
            ...
            try {
                $stmt->execute([$chipId, $batchName]);
                $successCount++;
            } catch (PDOException $e) {
                $errors[] = "Fehler bei $chipId: " . $e->getMessage();
                $skipCount++;
            }
        }
...
// ==========================================
// TRUSTED DEVICES (DEVICE REMEMBER) FUNKTIONEN
// ==========================================

/**
 * Gerät vertrauen und Token erstellen
 */
function trustDevice($pdo, $markerId, $deviceFingerprint, $deviceName = null, $durationDays = 90) {
    // Eindeutigen Token generieren
    $deviceToken = bin2hex(random_bytes(32));
    
    // Ablaufdatum berechnen (0 = unbegrenzt)
    $expiresAt = null;
    if ($durationDays > 0) {
        $expiresAt = date('Y-m-d H:i:s', strtotime("+$durationDays days"));
    }
    
    // Bestehendes Gerät prüfen und aktualisieren oder neues erstellen
    $stmt = $pdo->prepare("
        SELECT id FROM trusted_devices 
        WHERE marker_id = ? AND device_fingerprint = ?
    ");
    $stmt->execute([$markerId, $deviceFingerprint]);
    $existing = $stmt->fetch();
    ...
    if ($existing) {
        ...
        $stmt = $pdo->prepare("
            ...
            (marker_id, device_token, device_fingerprint, device_name, user_agent, ip_address, expires_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([
            $markerId,
 ...
 */
function validateDeviceToken($pdo, $markerId, $deviceToken, $deviceFingerprint) {
    $stmt = $pdo->prepare("
    ...
    ");
    $stmt->execute([$markerId, $deviceToken, $deviceFingerprint]);
    $device = $stmt->fetch();
...
function generateDeviceFingerprint() {
    ...
    // Einfacher Fingerprint basierend auf Browser-Eigenschaften
...
/**
 * Prüfen ob Device-Remember aktiviert ist
 */
function isDeviceRememberEnabled() {
    global $pdo;
    $settings = getSystemSettings();
    return isset($settings['device_remember_enabled']) && $settings['device_remember_enabled'] == '1';
}

/**
 * Prüfen ob Public Auth erforderlich ist
 */
function isPublicAuthRequired() {
    global $pdo;
    $settings = getSystemSettings();
    return isset($settings['require_public_auth']) && $settings['require_public_auth'] == '1';
}

/**
 * Vertrauensdauer in Tagen abrufen
 */
function getDeviceRememberDuration() {
    global $pdo;
    $settings = getSystemSettings();
    return isset($settings['device_remember_duration_days']) ? intval($settings['device_remember_duration_days']) : 90;
}

/**
 * Geo-Fence Funktionen
 */
...
/**
 * Ray-Casting Algorithm: Prüfen ob Punkt in Polygon liegt
 */
...
/**
 * Geo-Fence Berechtigung prüfen
 */
function checkGeofencePermission($geofence, $deviceType) {
    switch ($deviceType) {
        case 'customer':
            return !empty($geofence['allow_customer_devices']);
        case 'rental':
            return !empty($geofence['allow_rental_devices']);
        case 'storage':
            return !empty($geofence['allow_storage_devices']);
        case 'repair':
            return !empty($geofence['allow_repair_devices']);
        default:
            return true;
    }
}
