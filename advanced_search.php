...
// PERMISSION CHECK: Erweiterte Suche-Berechtigung erforderlich
requirePermission('advanced_search', 'Sie haben keine Berechtigung für die erweiterte Suche');
...
$canEdit = hasPermission('markers_edit');
?>
<!DOCTYPE html>
<html lang="de">
<head>
    ...
    <title>Erweiterte Suche - Marker System</title>
                ...
                <div class="search-sidebar">
                    <form method="GET" id="searchForm">
                        ...
                        <div class="filter-group">
                            ...
                            <input type="text" 
                                   name="search" 
                                   placeholder="Name, QR-Code, Seriennummer..." 
                        ...
                        <div class="filter-group">
                            ...
                            <div class="range-inputs">
                                <div>
                                    <label>Min:</label>
                                    <input type="number" name="fuel_min" min="0" max="100" value="<?= htmlspecialchars($filters['fuel_min']) ?>" placeholder="0">
                                </div>
                                <div>
                                    <label>Max:</label>
                                    <input type="number" name="fuel_max" min="0" max="100" value="<?= htmlspecialchars($filters['fuel_max']) ?>" placeholder="100">
                                </div>
                            </div>
                        </div>
                        ...
                        <div style="margin-top: 15px; padding-top: 15px; border-top: 2px solid var(--border-color);">
                            <form method="POST">
                                ...
                                <?php foreach ($filters as $key => $value): ?>
                                    <?php if (!empty($value)): ?>
                                        <input type="hidden" name="filters[<?= $key ?>]" value="<?= htmlspecialchars($value) ?>">
                                    <?php endif; ?>
                                <?php endforeach; ?>
                        ...
                        <div class="saved-searches">
                            ...
                            <?php foreach ($savedSearches as $saved): ?>
                                <div class="saved-search-item" onclick="window.location.href='?load_search=<?= $saved['id'] ?>'">
                                    <div style="flex: 1;">
                                        <div style="font-weight: 600; color: var(--primary-color);"><?= htmlspecialchars($saved['search_name']) ?></div>
                            ...
                            <?php else: ?>
                                ...
                                <?php foreach ($results as $marker): ?>
                                    ...
                                    <div class="result-card" onclick="focusMarker(<?= $marker['id'] ?>)" id="result-<?= $marker['id'] ?>">
                                        <h3 class="result-title"><?= htmlspecialchars($marker['name']) ?></h3>
                                        <div class="result-meta">
                                            <span><i class="fas fa-tag"></i> <?= htmlspecialchars($marker['category']) ?></span>
                                            <span><i class="fas fa-qrcode"></i> <?= htmlspecialchars($marker['qr_code']) ?></span>
                                            <?php if ($marker['serial_number']): ?>
                                                <span><i class="fas fa-barcode"></i> <?= htmlspecialchars($marker['serial_number']) ?></span>
                                            <?php endif; ?>
                                        ...
                                        <div class="result-actions">
                                            ...
                                            <!-- PERMISSION CHECK: Nur Bearbeiten-Button zeigen wenn Berechtigung vorhanden -->
                                            <?php if ($canEdit): ?>
