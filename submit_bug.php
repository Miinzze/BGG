...
try {
    ...
    if (isset($_FILES['screenshot']) && $_FILES['screenshot']['error'] === UPLOAD_ERR_OK) {
        ...
        $uploadDir = defined('UPLOAD_DIR') ? UPLOAD_DIR . 'bug_screenshots/' : 'uploads/bug_screenshots/';
        if (!is_dir($uploadDir)) {
    ...
    $stmt = $pdo->prepare("
        INSERT INTO bug_reports (
            ...
            browser_info, screenshot_path, priority, reported_by, status, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'offen', NOW())
    ");
    ...
    // E-Mail an Admin senden (falls Funktion existiert)
    if (function_exists('sendBugReportEmail')) {
        try {
            sendBugReportEmail($bugId, $title, $description, $email, $priority);
        } catch (Exception $e) {
            // E-Mail-Fehler nicht kritisch
            error_log('Bug Report Email Error: ' . $e->getMessage());
        }
    }
    
    // Aktivitätslog (falls Funktion existiert)
    if (function_exists('logActivity')) {
        try {
            logActivity('bug_report', "Bug gemeldet: $title", null);
        } catch (Exception $e) {
            // Log-Fehler nicht kritisch
            error_log('Bug Report Log Error: ' . $e->getMessage());
        }
    }
    ...
    error_log('Bug Report Database Error: ' . $e->getMessage());
    echo json_encode([
        'success' => false, 
        'message' => 'Datenbankfehler beim Speichern. Bitte kontaktieren Sie den Administrator.'
    ]);
} catch (Exception $e) {
    ...
    <body>
        <div class='container'>
            ...
            <div class='content'>
                ...
                <a href='http://" . ($_SERVER['HTTP_HOST'] ?? 'localhost') . "/bug_tickets.php?id=$bugId' class='btn'>
                    Bug ansehen
                </a>
            </div>
        </div>
