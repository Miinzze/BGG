...
// NFC-Marker: Spezielle Druckansicht
$isNFC = ($marker && isset($marker['marker_type']) && $marker['marker_type'] === 'nfc_chip' && !empty($marker['nfc_chip_id']));
$backupQR = isset($_GET['backup_qr']) && $_GET['backup_qr'] == '1';

if ($isNFC && !$backupQR) {
    // NFC-Druck-Modus - Spezielle Ansicht für NFC-Marker
    ?>
    <!DOCTYPE html>
    <html lang="de">
    <head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
        ...
        <title>NFC-Chip Info - <?= e($marker['name']) ?></title>
    ...
    <body>
        <div class="print-container">
            <div class="nfc-card">
                ...
                <div class="nfc-badge">
                    <i class="fas fa-microchip"></i> NFC-CHIP
                </div>
                
                <div class="nfc-id">
                    <?= e($marker['nfc_chip_id']) ?>
                </div>
                
                <?php if ($marker['category'] || $marker['serial_number']): ?>
                ...
                <div class="instructions">
                    <strong><i class="fas fa-info-circle"></i> NFC-Scan Anleitung:</strong>
                    <ul style="margin: 10px 0 0 20px; line-height: 1.8;">
                        <li>Halten Sie Ihr NFC-fähiges Smartphone an diesen NFC-Chip</li>
                        <li>Das Gerät öffnet automatisch die Geräteinformationen</li>
                        <li>NFC muss in den Geräteeinstellungen aktiviert sein</li>
                        <li>Funktioniert am besten mit Android-Geräten</li>
                    </ul>
                </div>
                
                <div class="backup-qr-hint">
                    <strong>Backup QR-Code verfügbar:</strong><br>
                    <?= e($marker['qr_code']) ?><br>
                    <small>(Falls NFC nicht verfügbar ist)</small>
                </div>
            </div>
        ...
        <div class="no-print" style="page-break-before: always;">
            <button onclick="window.print()" style="padding: 15px 30px; font-size: 16px; background: #17a2b8; color: white; border: none; border-radius: 5px; cursor: pointer; margin: 10px;">
                <i class="fas fa-print"></i> Drucken
            </button>
            <a href="print_qr.php?id=<?= $marker['id'] ?>&backup_qr=1" 
               style="padding: 15px 30px; font-size: 16px; background: #007bff; color: white; text-decoration: none; border-radius: 5px; display: inline-block; margin: 10px;">
                <i class="fas fa-qrcode"></i> Backup QR-Code drucken
            </a>
            <a href="view_marker.php?id=<?= $marker['id'] ?>" 
               style="padding: 15px 30px; font-size: 16px; background: #6c757d; color: white; text-decoration: none; border-radius: 5px; display: inline-block; margin: 10px;">
    ...
    <?php
    exit; // Verhindert die normale QR-Code-Ausgabe
}

// Branded QR-Code generieren mit höherer Auflösung für bessere Druckqualität
try {
    $qrApiUrl = generateBrandedQRCode($publicUrl, $logoPath, 600);
} catch (Exception $e) {
    // Fallback auf normale API mit höherer Auflösung
    $qrApiUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=600x600&margin=10&data=' . urlencode($publicUrl);
}
?>
<!DOCTYPE html>
<html lang="de">
...
<body>
    <?php if ($batch): ?>
        ...
        <?php foreach ($codes as $index => $batchQrCode): 
            ...
            try {
                $batchQrApiUrl = generateBrandedQRCode($batchPublicUrl, $logoPath, 600);
            } catch (Exception $e) {
                $batchQrApiUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=600x600&margin=10&data=' . urlencode($batchPublicUrl);
            }
        ?>
        <div class="print-container">
    ...
    <?php else: ?>
        ...
        <div class="print-container">
            ...
            <?php else: ?>
                ...
                <?php if ($isNFC && $backupQR): ?>
                <div style="background: #17a2b8; color: white; padding: 15px; border-radius: 8px; margin: 20px 0;">
                    <i class="fas fa-wifi"></i> <strong>NFC-MARKER - BACKUP QR-CODE</strong><br>
                    <small>Haupt-ID: <?= e($marker['nfc_chip_id']) ?></small>
                </div>
                <?php endif; ?>
