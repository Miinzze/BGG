...
if (isset($_GET['delete']) && isset($_GET['confirm'])) {
    ...
    try {
        ...
        if ($qrInfo['is_assigned'] && !$force) {
            // Marker-Namen holen
            $markerName = 'Unbekannt';
            if ($qrInfo['marker_id']) {
                $stmt = $pdo->prepare("SELECT name FROM markers WHERE id = ?");
                $stmt->execute([$qrInfo['marker_id']]);
                $markerName = $stmt->fetchColumn() ?: 'Unbekannt';
            }
            
            // Warnung anzeigen
            $showForceWarning = true;
            $warningQRCode = $qrCode;
            $warningMessage = "Dieser QR-Code ist dem Marker \"$markerName\" zugewiesen. Wenn Sie den QR-Code löschen, wird auch der Marker gelöscht!";
            
        } else {
            // Löschen (mit oder ohne Force)
            $pdo->beginTransaction();
            
            if ($force && $qrInfo['is_assigned'] && $qrInfo['marker_id']) {
                ...
                $stmt = $pdo->prepare("
                    ...
                    WHERE id = ?
                ");
                $stmt->execute([$_SESSION['user_id'], $qrInfo['marker_id']]);
                
                logActivity('marker_deleted_by_qr', "Marker automatisch gelöscht durch QR-Code Löschung", $qrInfo['marker_id']);
            }
            ...
            logActivity('qr_code_deleted', "QR-Code '$qrCode' gelöscht" . ($force ? ' (inkl. Marker)' : ''));
            
            $message = "QR-Code erfolgreich gelöscht!" . ($force ? " Der zugewiesene Marker wurde in den Papierkorb verschoben." : "");
            $messageType = 'success';
        }
...
<body>
    ...
    <div class="main-container">
        <div class="content-wrapper">
            ...
            <div class="alert alert-warning" style="border-left: 4px solid #ffc107; background: #fff3cd;">
                <h3><i class="fas fa-exclamation-triangle"></i> Warnung: QR-Code ist zugewiesen</h3>
                ...
                <div style="background: white; padding: 15px; border-radius: 5px; margin: 15px 0;">
                    ...
                    <ul style="margin: 10px 0 0 20px;">
                        <li>Der QR-Code wird aus dem Pool entfernt</li>
                        <li>Der zugewiesene Marker wird in den Papierkorb verschoben</li>
                        <li>Dieser Vorgang kann NICHT rückgängig gemacht werden</li>
                    </ul>
                </div>
                <div style="margin-top: 20px; display: flex; gap: 10px;">
                    <a href="qr_code_list.php?delete=<?= urlencode($warningQRCode) ?>&confirm=1&force=1&csrf_token=<?= $_SESSION['csrf_token'] ?>" 
                    class="btn btn-danger btn-large"
                    onclick="return confirm('Wirklich löschen? QR-Code UND Marker werden entfernt!')">
                        <i class="fas fa-trash-alt"></i> Ja, QR-Code UND Marker löschen
                    </a>
                    <a href="qr_code_list.php" class="btn btn-secondary btn-large">
